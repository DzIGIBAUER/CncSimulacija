...
/// <summary>
/// Klasa koja pruža metoda za pronalaženje convex hull-a pomoću GrahamScan algoritma https://en.wikipedia.org/wiki/Graham_scan.
/// </summary>
class GrahamScan {
    public static IList<Vector3> GrahamScanCompute(IList<Vector3> initialPoints)
    {
        ...
        var sortQuery = Enumerable.Range(0, initialPoints.Count)
            ...
            .Select((i) => new KeyValuePair<double, Vector3>(Math.Atan2(initialPoints[i].y - initialPoints[iMin].y, initialPoints[i].y - initialPoints[iMin].x), initialPoints[i]))
            ...
            else
            {
                while (true)
                {
                    ...
                    else if (flag == RemovalFlag.EndPoint)
                    {
                        ...
                        throw new System.Exception("Unknown RemovalFlag");
                }
            }
            if (keepNewPoint)
            {
